...
def bagging(trees):
    ...
    for t in range(trees):
        ...
        h_t = algo.ID3_Algo(train_data_sampled ,train_data_sampled ,columns[:-1],split_method,[])
...
#In[]:
for i in range(500):
    train_error, test_error = bagging(i+1)
    train_errors.append(train_error)
    test_errors.append(test_error)
    print("Trees:- "+str(i+1)+". Training error:- "+str(train_error)+". Test Error:- "+str(test_error))
