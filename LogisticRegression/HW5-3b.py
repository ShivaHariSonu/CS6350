...
def grad_dw(x,y,w):
    grad = ((np.exp(-y*w.T*x)*(-y*x))/(1+np.exp(-y*w.T*x)))+w
    return grad


d = 0.2
epochs = 100
for learning_rate in [0.001,0.005,0.1,0.5,1]:
    print("For learning rate :- {}".format(learning_rate))
    ...
    for epoch in range(epochs):
        ...
        for j in range(len(X_train)):
            dw = grad_dw(X_train.iloc[j],Y_train[j],weights)
    ...
    for i in range(len(X_train)):
        ...
        if pred>=0.5 and Y_train[i]==1:
            correct_pred+=1
    print("Train Error {} and accuracy is {}".format((len(X_train)-correct_pred)/len(X_train),(correct_pred/len(X_train)*100)))
    correct_pred = 0
    for i in range(len(X_test)):
        ...
        if pred>=0.5 and Y_test[i]==1:
            correct_pred+=1
    print("Test Error {} and accuracy is {}".format((len(X_test)-correct_pred)/len(X_test),(correct_pred/len(X_test)*100)))
    print("*"*50)
