...
########Filling the missing values with majority element for training data######

maximum_fea = {}

for col in columns:
    most_frequent = train_data[col].replace("unknown", np.nan).mode()[0]
    maximum_fea[col] = most_frequent
    train_data[col] = train_data[col].replace("unknown", most_frequent)
...
########Filling the missing values with majority element for test data######

maximum_fea = {}

for col in columns:
    most_frequent = train_data_backup[col].replace("unknown", np.nan).mode()[0]
    maximum_fea[col] = most_frequent
    test_data[col] = test_data[col].replace("unknown", most_frequent)
############################################
